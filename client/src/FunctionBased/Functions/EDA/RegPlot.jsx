...
function RegPlot({
    ...
    useEffect(() => {
        const handleChatbotRegPlot = (event) => {
            const { x_var, y_var, title } = event.detail;
            console.log(
                "🤖 Chatbot triggered regression plot generation with params:",
                event.detail,
            );
            ...
            generatePlotWithParams(
                Array.isArray(x_var) ? x_var : [x_var],
                y_var,
                title,
            );
        };
        ...
        return () => {
            window.removeEventListener(
                "chatbotGenerateRegPlot",
                handleChatbotRegPlot,
            );
        };
    }, [csvData]);

    // Generate plot with direct parameters (for chatbot)
    const generatePlotWithParams = async (xVar, yVar, title) => {
        try {
            setLoading(true);
            setEchartsData([]);
            setError(null);
