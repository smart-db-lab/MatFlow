    ...
    const fetchData = async () => {
      try {
        ...
        const hasModels = tempModels.some((model) => {
          ...
          return (
            model[datasetName] && Object.keys(model[datasetName]).length > 0
          );
        });
  ...
  if (!allDatasetName || allDatasetName.length === 0 || notFound)
    return (
      <div className="flex flex-col items-center justify-center h-64 gap-3">
        ...
        <p className="text-sm text-gray-400">Build a model first to see evaluation results.</p>
      </div>
    );
  return (
      ...
      <div className="flex-1 flex gap-5 overflow-hidden">
        ...
        <div className="w-[340px] min-w-[300px] max-w-[400px] overflow-y-auto">
          <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm">
            ...
            <div className="px-5 py-4 space-y-5">
              ...
              {display_type === 'Graph' ? (
                <div>
                  ...
                  <SingleDropDown
                    columnNames={['Vertical', 'Horizontal', 'Radar']}
                    initValue={'Vertical'}
            ...
            <div className="px-5 py-4 border-t border-gray-100 space-y-2.5">
              {test_dataset && (
                <button
                  className="w-full px-4 py-2.5 text-sm font-semibold rounded-lg bg-primary hover:bg-primary-dark active:bg-primary-dark text-white shadow-sm shadow-primary/20 hover:shadow-md hover:shadow-primary/25 transition-all duration-150"
                  onClick={handleSave}
                >
                  Generate Results
                </button>
              ...
              {(columnDefs || graphData) && (
                <button
                  ...
                  onClick={() => dispatch(setActiveFunction('Model Prediction'))}
                >
                  Model Prediction
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          {columnDefs && columnDefs.length > 0 && (
            <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm overflow-hidden">
              ...
              <div className="px-5 py-3.5 border-b border-gray-100 flex items-center justify-between gap-4">
                <h3 className="text-[13px] font-semibold text-gray-800 uppercase tracking-wider flex items-center gap-2">
                  <svg className="w-4 h-4 text-gray-400" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M3 10h18M3 14h18m-9-4v8m-7 0h14a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg>
                  Performance Metrics
                </h3>
                <div className="w-56">
                  <MultipleDropDown
                      ...
                      {columnDefs.map((col, i) => (
                        <th
                          ...
                          className={`px-5 py-3 text-[11px] font-semibold uppercase tracking-wider text-gray-500 text-left border-b border-gray-100 ${
                            col?.field === 'name' ? 'sticky left-0 bg-gray-50/80 z-10' : ''
                          }`}
                        >
                          {col?.headerName || ''}
                        </th>
                    ...
                    {file && file.map((row, rowIdx) => (
                      ...
                      >
                        {columnDefs.map((col, colIdx) => {
                          ...
                          let badgeClass = '';
                          if (isNumber && !isName) {
                            if (value >= 0.9) badgeClass = 'text-emerald-700 bg-emerald-50 border-emerald-100';
                            else if (value >= 0.7) badgeClass = 'text-amber-700 bg-amber-50 border-amber-100';
                            else badgeClass = 'text-red-700 bg-red-50 border-red-100';
                          }
                          return (
                            <td
                            ...
                            >
                              {isName ? (
                                <div className="flex items-center gap-2.5">
                                  <div className="w-2 h-2 rounded-full flex-shrink-0" style={{ backgroundColor: ['#ff6384','#36a2eb','#ffce56','#4bc0c0','#9966ff','#ff9f40'][rowIdx % 6] }} />
              ...
              <div className="px-5 py-2.5 border-t border-gray-100 bg-gray-50/50">
                <p className="text-[11px] text-gray-400 font-medium">
                  {file ? file.length : 0} model{file && file.length !== 1 ? 's' : ''} &middot; {columnDefs.length} metric{columnDefs.length !== 1 ? 's' : ''}
                </p>
              </div>
            </div>
          )}
          ...
          {graphData && (
            <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm overflow-hidden">
              <div className="px-5 py-3.5 border-b border-gray-100">
                <h3 className="text-[13px] font-semibold text-gray-800 uppercase tracking-wider">Visualization</h3>
              </div>
              <div className="p-4">
                <Plot
                  data={(graphData?.data || []).map((trace) =>
                    trace?.type === 'bar'
                      ? {
                          ...trace,
                          // Slimmer bars for a cleaner comparison chart look
                          width: 0.45,
                        }
                      : trace
                  )}
                  layout={applyPlotlyTheme({
                    ...(graphData?.layout || {}),
                    // Increase gaps so bars don't look too fat
                    bargap: 0.45,
                    bargroupgap: 0.2,
                  })}
          ...
          {!columnDefs && !graphData && (
            <div className="flex flex-col items-center justify-center h-full min-h-[400px] gap-3">
              ...
              <p className="text-xs text-gray-400">Configure settings and click <span className="font-semibold text-primary">"Generate Results"</span></p>
            </div>
          )}
        </div>
      </div>
