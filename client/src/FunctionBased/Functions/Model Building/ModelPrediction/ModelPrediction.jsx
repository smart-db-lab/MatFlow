...
function ModelPrediction({ csvData }) {
    ...
    const parseClassificationReport = (reportText) => {
        ...
        const accuracyRegex =
            /^accuracy\s+(-?\d+(?:\.\d+)?)\s+(\d+(?:\.\d+)?)$/;

        lines.forEach((rawLine) => {
            const line = rawLine.trim();
            if (
                line.toLowerCase().includes("precision") &&
                line.toLowerCase().includes("recall")
            ) {
                return;
            }

            const acc = line.match(accuracyRegex);
            if (acc) {
                accuracy = {
                    score: Number(acc[1]),
                    support: Number(acc[2]),
                };
                return;
            }
            ...
            const row = {
                ...
                f1Score: Number(match[4]),
                support: Number(match[5]),
            };

            if (label === "macro avg" || label === "weighted avg") {
        ...
        if (!classRows.length && !summaryRows.length && !accuracy) return null;
        return { classRows, summaryRows, accuracy };
    };
        ...
        try {
            const resolvedWorkspaceId =
                ...
                extractWorkspaceIdFromPath(
                    modelData?.workspace_metadata_file,
                ) ||
    ...
    const renderTableCard = (title, rowData, colKeys) => (
        <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm">
            <div className="px-5 py-3.5 border-b border-[#99f6e4] bg-[#f0fdfa] flex items-center justify-between gap-3">
                <h3
                    className={`${FE_SECTION_TITLE_CLASS} !mb-0 text-[#115e59]`}
                >
                    {title}
                </h3>
    ...
    const renderGraphCard = (_title, graphJson) => {
        ...
        // ECharts format has 'series', 'xAxis'/'yAxis', 'polar', 'gauge' or other ECharts-specific properties
                                            ...
                                            <div className="bg-white rounded-xl border border-gray-200/80 shadow-sm overflow-hidden">
                                                ...
                                                <div className="p-5">
                                                    {(() => {
                                                        ...
                                                        if (!parsed) {
                                                            return (
                                                                <pre className="text-[13px] leading-relaxed text-gray-800 whitespace-pre-wrap font-mono bg-gray-50 p-4 rounded-lg border border-gray-100">
                                                                    {data}
                                                                </pre>
                                                            );
                                                        }
                                                                    ...
                                                                    <table className="w-full min-w-[640px]">
                                                                        <thead>
                                                                            <tr className="bg-teal-50/70">
                                                                                <th className="px-4 py-3 text-left text-xs uppercase tracking-wide text-gray-800 font-bold border-b border-teal-200">
                                                                                    Class
                                                                                </th>
                                                                                <th className="px-4 py-3 text-right text-xs uppercase tracking-wide text-gray-800 font-bold border-b border-teal-200">
                                                                                    Precision
                                                                                </th>
                                                                                <th className="px-4 py-3 text-right text-xs uppercase tracking-wide text-gray-800 font-bold border-b border-teal-200">
                                                                                    Recall
                                                                            ...
                                                                            {classRows.map(
                                                                                (
                                                                                    ...
                                                                                    >
                                                                                        ...
                                                                                        <td className="px-4 py-2.5 text-sm text-right text-gray-700 font-mono">
                                                                                            {row.precision.toFixed(
                                                                                                3,
                                                                                            )}
                                                                                        </td>
                                                                                        <td className="px-4 py-2.5 text-sm text-right text-gray-700 font-mono">
                                                                            ...
                                                                            <tbody>
                                                                                {summaryRows.map(
                                                                                    (
                                                                                        ...
                                                                                        <tr
                                                                                            key={`${row.label}-summary-${idx}`}
                                                                                            className="border-t border-gray-100 bg-gray-50/60"
                                                                                        >
                                                                                            <td className="px-4 py-2.5 text-sm text-gray-800 font-semibold">
                                                                                                {
                                                                                                    row.label
                                                                                                }
                                                                                            </td>
