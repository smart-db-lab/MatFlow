...
const STAGES = [
  { id: 'split', label: 'Split Dataset', functionName: 'Split Dataset', key: 'splitted_dataset' },
  { id: 'build', label: 'Build Model', functionName: 'Build Model', key: 'models', requires: 'split' },
  { id: 'evaluate', label: 'Model Evaluation', functionName: 'Model Evaluation', key: 'models', requires: 'build' },
  { id: 'predict', label: 'Model Prediction', functionName: 'Model Prediction', key: 'models', requires: 'build' },
  { id: 'models', label: 'Models', functionName: 'Models', key: 'models', alwaysAccessible: true },
];
...
function ModelBuildingWorkflow({ csvData }) {
  ...
  const checkCompletionStatus = async () => {
    try {
      ...
      const splitData = await fetchDataFromIndexedDB('splitted_dataset');
      const splitComplete = splitData && splitData.length > 0;
      
      // Check models completion
      const modelsData = await fetchDataFromIndexedDB('models');
  ...
  useEffect(() => {
    const handleStageComplete = async (event) => {
      ...
      if (stage === 'split') {
        ...
        localStorage.setItem('activeFunction', 'Build Model');
        window.dispatchEvent(
          new CustomEvent('forceFunctionSelection', {
            detail: { nodeId: functionNodeMap['Build Model'], label: 'Build Model' },
          })
        );
      } else if (stage === 'build') {
        dispatch(setActiveFunction('Model Evaluation'));
        localStorage.setItem('activeFunction', 'Model Evaluation');
        window.dispatchEvent(
          new CustomEvent('forceFunctionSelection', {
            detail: { nodeId: functionNodeMap['Model Evaluation'], label: 'Model Evaluation' },
          })
        );
      }
    };
  ...
  const handleStageClick = (stage) => {
    ...
    if (stage.alwaysAccessible || completionStatus[stage.id] || currentStage === stage.id) {
      dispatch(setActiveFunction(stage.functionName));
      localStorage.setItem('activeFunction', stage.functionName);
      ...
      <Box className="w-full bg-white px-4 py-2 mb-2">
        <StyledStepper activeStep={getActiveStep()} alternativeLabel>
          {STAGES.map((stage, index) => {
            ...
            const isAccessible = !isDisabled && (isCompleted || isActive || isStageAccessible(stage));
