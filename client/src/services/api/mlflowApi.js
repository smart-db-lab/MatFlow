...
export const mlflowApi = {
  ...
  auth: {
    ...
    verifyCode: async (email, code) => {
      const response = await apiFetch(`${API_BASE_URL}/api/verify-code/`, {
        method: 'POST',
    ...
    refreshToken: async (refreshToken) => {
      const response = await apiFetch(`${API_BASE_URL}/api/auth/token/refresh/`, {
        method: 'POST',
  ...
  articles: {
    ...
    updateJournal: async (id, data) => {
      const response = await apiFetch(`${API_BASE_URL}/api/journals/${id}/`, {
        method: 'PUT',
        body: JSON.stringify(data),
      });
      return await parseResponse(response);
    ...
    updateConference: async (id, data) => {
      const response = await apiFetch(`${API_BASE_URL}/api/conferences/${id}/`, {
        method: 'PUT',
        body: JSON.stringify(data),
      });
      return await parseResponse(response);
    ...
    updateBook: async (id, data) => {
      const response = await apiFetch(`${API_BASE_URL}/api/books/${id}/`, {
        method: 'PUT',
        body: JSON.stringify(data),
      });
      return await parseResponse(response);
    ...
    updatePatent: async (id, data) => {
      const response = await apiFetch(`${API_BASE_URL}/api/patents/${id}/`, {
        method: 'PUT',
        body: JSON.stringify(data),
      });
      return await parseResponse(response);
    ...
    updateDataset: async (id, data, file = null) => {
      ...
      const response = await apiFetch(`${API_BASE_URL}/api/datasets/${id}/`, {
        method: 'PUT',
        headers,
        body: requestBody,
      });
  ...
  services: {
    getAll: async (serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/our-services/`, serviceKey);
    ...
    update: async (id, formData, serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/our-services/${id}/`, serviceKey);
      const response = await apiFetch(url, {
        method: 'PATCH',
        body: formData,
      });
  ...
  landing: {
    getHeroImages: async (serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/hero-image/`, serviceKey);
      const response = await apiFetch(url);
      const data = await parseResponse(response);
      return extractData(data);
    },
    ...
    getHeaderSection: async (serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/header-section/`, serviceKey);
      const response = await apiFetch(url);
      const data = await parseResponse(response);
      const extracted = extractData(data);
      return Array.isArray(extracted) && extracted.length > 0 ? extracted[0] : null;
    },

    getSupportLogos: async (serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/support-logo/`, serviceKey);
      const response = await apiFetch(url);
      const data = await parseResponse(response);
      return extractData(data);
    },
    ...
    updateHeroImage: async (id, formData, serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/hero-image/${id}/`, serviceKey);
      const response = await apiFetch(url, {
        method: 'PATCH',
        body: formData,
      });
    ...
    updateSupportLogo: async (id, formData, serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/support-logo/${id}/`, serviceKey);
      const response = await apiFetch(url, {
        method: 'PATCH',
        body: formData,
      });
    ...
    updateHeaderSection: async (id, formData, serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/header-section/${id}/`, serviceKey);
      // If formData is not FormData, stringify it for JSON request
      const body = formData instanceof FormData ? formData : JSON.stringify(formData);
      const response = await apiFetch(url, {
        method: 'PATCH',
  ...
  faq: {
    ...
    update: async (id, faqData, serviceKey = 'mlflow') => {
      const url = withServiceKey(`${API_BASE_URL}/api/faq/${id}/`, serviceKey);
      const response = await apiFetch(url, {
        method: 'PATCH',
        body: JSON.stringify(faqData),
      });
  ...
  visitors: {
    trackVisit: async (visitorId, visitorType = 'guest') => {
      const response = await apiFetch(`${API_BASE_URL}/api/track-visit/`, {
        method: 'POST',
        body: JSON.stringify({ visitor_id: visitorId, visitor_type: visitorType }),
      });
      return await parseResponse(response);
    },

    getStats: async () => {
      const response = await apiFetch(`${API_BASE_URL}/api/visitor-stats/`);
      return await parseResponse(response);
    },
  },
};
