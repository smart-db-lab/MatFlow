with open("client/src/FunctionBased/Functions/Dataset/DatasetCorrelation.jsx", "r") as f:
    text = f.read()
...
# Remove buildHeatmapFilename, saveHeatmapToProject, downloadAndPersistHeatmap
import re
text = re.sub(r'const buildHeatmapFilename = \(\) => \{.+?toast\.error\("Failed to download/save heatmap\. Please try again\.", \{\n\s+autoClose: 3200,\n\s+\}\);\n\s+\};\n', '', text, flags=re.DOTALL)

# Replace the plot rendering section
old_render = """) : plotlyData ? (
...
new_render = """) : echartsData.length > 0 ? (
...
with open("client/src/FunctionBased/Functions/Dataset/DatasetCorrelation.jsx", "w") as f:
    f.write(text)
print("done")
